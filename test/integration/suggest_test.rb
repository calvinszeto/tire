...
module Tire
  ...
  class SuggestIntegrationTest < Test::Unit::TestCase
    ...
    context "Suggest" do
      teardown { Tire.index('suggest-test').delete }

      should "add 'suggest' field to the result item" do
        # Tire::Configuration.logger STDERR, :level => 'debug'
        s = Tire.search('articles-test') do
					suggest "twoo", :term => {:field => "title"}
        end

				suggestion = s.results.suggestions.suggestion.first

				assert_equal 'twoo', suggestion.text
				assert_equal 'two', suggestion.options.first.text
      end
